...
while True:
    ...
    for pin in [BUTTON_DEC_PIN, BUTTON_INC_PIN, BUTTON_LED_TOGGLE_PIN]:
        ...
        if current_state == wiringpi.LOW and previous_button_state[pin] == wiringpi.HIGH:
            if pin == BUTTON_DEC_PIN:
                LED_BRIGHTNESS = max(MIN_BRIGHTNESS, LED_BRIGHTNESS - BRIGHTNESS_STEP)
                LUX_GOAL = max(0, LUX_GOAL - BRIGHTNESS_STEP)  # Decrease LUX_GOAL
                wiringpi.softPwmWrite(LED_PIN, LED_BRIGHTNESS)
                print(f"Button on pin {pin} Pressed! Decreased LED Brightness to {LED_BRIGHTNESS}% and LED Brightness Goal to {LUX_GOAL}")
            elif pin == BUTTON_INC_PIN:
                LED_BRIGHTNESS = min(MAX_BRIGHTNESS, LED_BRIGHTNESS + BRIGHTNESS_STEP)
                LUX_GOAL = min(1000, LUX_GOAL + BRIGHTNESS_STEP)  # Increase LUX_GOAL
                wiringpi.softPwmWrite(LED_PIN, LED_BRIGHTNESS)
                print(f"Button on pin {pin} Pressed! Increased LED Brightness to {LED_BRIGHTNESS}% and LED Brightness Goal to {LUX_GOAL}")
    ...
    #Volledig werkende code met LED